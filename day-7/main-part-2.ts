...
const totalNeededSize = 30_000_000;

const totalUsedSpace = fileMapping['size'];
const freeSpace = totalDiskSize - totalUsedSpace;
const spaceNeededToBeFreed = totalNeededSize - freeSpace;

let currentBest = totalDiskSize;

const recursivelyFindPerfectDirectorySize = (currentMapping: any) => {
  const directoriesAndFiles = Object.keys(currentMapping);

  if (currentMapping['size'] < currentBest && currentMapping['size'] >= spaceNeededToBeFreed) {
    currentBest = currentMapping['size'];
  }

  const directories = directoriesAndFiles.filter((item) => isNaN(Number(currentMapping[item])));
  for (const directory of directories) {
    recursivelyFindPerfectDirectorySize(currentMapping[directory]);
  }
}

recursivelyFindPerfectDirectorySize(fileMapping);

console.log(spaceNeededToBeFreed);
